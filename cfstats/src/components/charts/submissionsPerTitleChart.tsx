...
const LogarithmicSubmissionsPerTitleChart = () => {
    const chartData = Object.keys(data).map((key) => {
        let [min, max] = data[key as keyof typeof data];
        if (min && max) {
    ...
    return (
        <Card className="w-full max-w-3xl">
            <CardHeader>
                <CardTitle>Logarithmic Average Number of Solved Problems</CardTitle>
            </CardHeader>
            <CardContent className="h-[500px]">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart
...
export default { SubmissionsPerTitleChart, LogarithmicSubmissionsPerTitleChart };